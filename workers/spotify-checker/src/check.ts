...
const discordRetryAfterMs = async (
  response: Response,
  ...
  return pRetry(
    ...
    },
    {
      ...
      onFailedAttempt: ({
        ...
        console.warn(
          `${context} attempt ${attemptNumber} failed (${retriesLeft} retries left): ${error.message}`,
        );
      },
    },
  );
...
const discordFetchOk = (
  url: string | URL,
  init: RequestInit,
  context: string,
): Promise<Response> => fetchWithRetry(url, init, context, discordRetryAfterMs);
...
const spotifyFetchJson = pThrottle({
  ...
  onDelay: () => {
    console.log(
      `Spotify rate limit reached (${SPOTIFY_MAX_REQUESTS}/${SPOTIFY_RATE_WINDOW_MS}ms), queuing request`,
    );
  },
})(
  async <T>(
    ...
    init: RequestInit,
    context: string,
  ): Promise<T> => fetchJson<T>(url, init, context),
);
...
const spotifyRefreshClient = (env: Env): arctic.Spotify =>
  new arctic.Spotify(
    ...
    'https://localhost/auth/spotify/callback',
  );
...
const refreshSpotifyAccessToken = async (
  ...
  const tokens =
    await spotifyRefreshClient(env).refreshAccessToken(refreshToken);
...
const getFollowedArtists = async (
  ...
  while (url !== null) {
    page++;
    const data: SpotifyApi.UsersFollowedArtistsResponse =
      await spotifyFetchJson(
        url,
  ...
  console.log(
    `Fetched ${artists.length} followed artist(s) across ${page} page(s)`,
  );
  return artists;
};
  ...
  const loaded = await Promise.all(
    users.map(async (user) => {
      try {
        ...
        console.error(
          `Failed to refresh token for spotify user ${user.spotifyUserId}:`,
          error,
        );
        return null;
      }
...
): boolean => notifiedIds.has(`${albumId}:${phase}`);
...
// Normalises Spotify's YYYY, YYYY-MM, or YYYY-MM-DD release_date to YYYY-MM-DD.
const getReleaseDateString = (
  album: SpotifyApi.AlbumObjectSimplified,
): string => {
  const [year, month = '01', day = '01'] = album.release_date.split('-');
  if (album.release_date_precision === 'day') return `${year}-${month}-${day}`;
  if (album.release_date_precision === 'month') return `${year}-${month}-01`;
  return `${year}-01-01`;
};

const isAlbumReleased = (
  album: SpotifyApi.AlbumObjectSimplified,
  now = new Date(),
): boolean => getReleaseDateString(album) <= getLocalDateString(now);

const isAlbumPresave = (
  album: SpotifyApi.AlbumObjectSimplified,
  now = new Date(),
): boolean => !isAlbumReleased(album, now);

// Released strictly before today (local time) — i.e. back catalog, not a same-day drop.
const isAlbumBackCatalog = (
  album: SpotifyApi.AlbumObjectSimplified,
  now = new Date(),
): boolean => getReleaseDateString(album) < getLocalDateString(now);
...
const formatTrackList = (
  ...
  const truncated = listedTrackCount - shown.length;
  if (truncated > 0) {
    lines.push(`…and ${truncated} more`);
  }

  const notYetListed = totalAlbumTracks - listedTrackCount;
  if (notYetListed > 0) {
...
const getAlbumTracks = async (
  ...
  const query = new URLSearchParams({
    market: 'AU',
    limit: String(MAX_TRACKLIST_TRACKS),
  });
  const url = `https://api.spotify.com/v1/albums/${album.id}/tracks?${query.toString()}`;
  ...
  return { tracks: data.items, listedTrackCount: data.total };
};
  ...
  embeds: [
    {
      ...
      author: {
        ...
        ...(followedArtist.images.length > 0 && {
          icon_url: followedArtist.images[0].url,
        }),
      },
      ...
      fields: [
        {
          name: album.artists.length > 1 ? 'Artists' : 'Artist',
          value: formatArtistNames(album.artists),
          inline: false,
        },
        ...
        {
          name: 'Status',
          value: phase === 'presave' ? 'Pre-save' : 'Released',
          inline: true,
        },
        ...(tracks.length > 0
          ? [
...
const postToDiscord = pThrottle({
  ...
  onDelay: () => {
    console.log(
      `Discord rate limit reached (${DISCORD_MAX_REQUESTS}/${DISCORD_RATE_WINDOW_MS}ms), queuing webhook`,
    );
  },
...
const processArtist = async (
  ...
  try {
    const { notifiedIds: notifiedAlbums, isFirstSight } =
      await getNotifiedAlbums(kv, artist.id);
    ...
    for (const album of items) {
      ...
      if (
        isAlbumPresave(album, now) &&
        !hasNotified(notifiedAlbums, album.id, 'presave')
      ) {
        phases.push('presave');
      }
      if (
        isAlbumReleased(album, now) &&
        !hasNotified(notifiedAlbums, album.id, 'release')
      ) {
        phases.push('release');
      }
      ...
      const suppressPost =
        seed || (isFirstSight && isAlbumBackCatalog(album, now));
      ...
      const { tracks, listedTrackCount } = suppressPost
        ? { tracks: [], listedTrackCount: 0 }
        : await getAlbumTracks(accessToken, album);
      ...
      for (const phase of phases) {
        ...
        if (suppressPost) {
          ...
          console.log(
            `Notifying (${phase}): ${releaseSummary} → ${discordUserIds.join(', ')}`,
          );
          await postToDiscord(
            webhookUrl,
            buildAlbumEmbed(
...
const buildArtistMap = async (
  users: UserWithToken[],
): Promise<Map<string, ArtistSubscribers>> => {
  const byArtistId = new Map<string, ArtistSubscribers>();

  for (const user of users) {
    const artists = await getFollowedArtists(user.accessToken);
    for (const artist of artists) {
      const artistSubscribers = byArtistId.getOrInsertComputed(
        artist.id,
        () => ({
          artist,
          discordUserIds: new Set([user.discordUserId]),
          accessToken: user.accessToken,
        }),
      );

      artistSubscribers.discordUserIds.add(user.discordUserId);
    }
  }
...
export const runCheck = async (
  ...
  console.log(
    `Starting spotify check (trigger: ${trigger}${seed ? ', seed mode' : ''})`,
  );
    ...
    [...artistMap.values()].map(({ artist, discordUserIds, accessToken }) =>
      limit(() =>
        processArtist(
          ...
          env.POSTED_ALBUMS,
          seed,
          Array.from(discordUserIds),
        ),
      ),
    ),
  );

  const totalRecorded = recordedPerArtist.reduce(
    (sum, count) => sum + count,
    0,
  );
  const artistsWithNewAlbums = recordedPerArtist.filter(
    (count) => count > 0,
  ).length;
